...
namespace GameLib
{
    public class CollisionDetection
    {
        ...
        public void DetectAndResolve(Entity root)
        {
            groups = new CollisionGroups(MaxGroup);
        ...
        private void DetectAndResolveAllGroups()
        {
            ...
            if(groups.IsSingleGroup(out list))
        ...
        private void FindAllCollisionObjs(Entity node)
        {
            Debug.Assert(node != null);
            var collisionObj = node as CollisionObj;
            if (collisionObj != null)
                groups.Add(collisionObj);

            var enumerable = node as IEnumerable<Entity>;
            if(enumerable != null)
            {
